...
########################################################
#
# We will use the sample data provided in the Sample Data folder to determine biodiversity hotspots.
#
#
# Let's first explore this dataset.
#
...
#        1 15 370
#
# obs_plants is a dataframe of all species observations. The column "species" gives the unique species ID,
# the column "x" is the X coordinate and the column "y" is the Y coordinate of each grid cell.
# Double observations (the same species occuring multiple times in the same grid cell) have been removed.
#
...
#                 7 Group2
#
# plant_species is a dataframe listing all species we want to take into account for determining the biodiversity hotspots.
# If a species does not occur in this list, but does occur in the observation dataframe, it is ignored.
# The "accepted_species" column gives the unique ID of each species, the column "group" gives the species group of each species.
# Hotspots are determined for each group seperately.
#
...
# We'll go into each parameter of this function and what it does.
# A description of each parameter can also be found in the HotspotStatus.R script.
#
hotspotResult = HotspotStatus(species = species,               # species is a vector of observed species
                              x = x,                           # x is a vector of X coordinates of the observed species
                              y = y,                           # y is a vector of Y coordinates of the observed species
                              observationTable = obs_plants,   # Optional. Dataframe containing the above three vectors. 
                                                               # The vectors can also be provided seperately.
                              speciesList = accepted_species,  # Optional. Vector of species to be taken into account for determining the hotspots.
                                                               # If not provided all observed species will be considered.
                              speciesGroups = group,           # Optional. Vector of groups the species are part of. Hotspots will be determined for each group.
                                                               # If not provided, all species will be considered to be in one group.
                              speciesTable = plant_species,    # Optional. Dataframe containing the above two vectors.
                                                               # The vectors can also be provided seperately.
                              gridcellList = Gridcells,        # Vector of all grid cells. Grid cells need to be the X and Y coordinate appended.
                              numberOfHotspots = 2,            # The number of hotspots per group to be determined.
                                                               # If there are ties, the actual number of hotspots might be slightly higher.
                              subTopMode = "amount",           # If and how the subtop should be calculated ("none", "amount", "sd", "2sd").
                                                               # This subtop is used to extend the hotspot to be larger than a single grid cell.
...
# 2. Determine for each species group seperately whether each grid cell belongs in its subtop (in this case, 15 subtop grid cells will be determined per group).
# 3. For each species group seperately, determine the grid cell with the single highest biodiversity.
# 4. Find subtop grid cells adjecent to this cell, adding these to the hotspot.
...
# The column "Hotspot_" followed by the group name ("Hotspot_Group1" in this case) contains whether each grid cell is the center of a hotspot for that species group.
# The column "subtop_" followed by the group name ("subtop_Group1" in this case) contains whether each grid cell is within the subtop for that species group.
